...
条件に合うメールを自動で検知し、Google Spreadsheet に記録するツールです。

問い合わせ、予約、通知、申込メールなどを手作業で確認・転記している場合、確認漏れや入力ミスが起きやすくなります。このツールは、メールの件名や送信元を条件に必要なメールだけを抽出し、送信者・件名・受信日時などをスプレッドシートへ自動記録します。
...
- 問い合わせメールを一覧化したい
- 予約・申込メールを Google Spreadsheet に自動転記したい
- メール確認後の手作業を減らしたい
- 小さな業務フローを低コストで自動化したい

## 主な機能

- Gmail / IMAP 対応のメール取得
- 件名、送信元によるフィルタリング
- Google Spreadsheet への自動書き込み
- YAML による条件設定
- Docker / cron による定期実行
- ログ出力
...
メール確認とスプレッドシート転記を組み合わせた、業務自動化の実装サンプルです。
実際の業務では、メール本文からの項目抽出、通知連携、集計用シートの設計などに拡張できます。
